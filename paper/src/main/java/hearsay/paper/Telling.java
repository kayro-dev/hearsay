 ...
 */
record Telling(int tellerId, int listenerId, double newConfidence, Claim claim) {
     ...
     *              stands for. Scarcity and abundance are drawn in opposite colours, only
     *              the claim on display is marked when it takes, and the event carries an
     *              id rather than a claim
     */
        ...
        for (Event event : events) {
            if (event instanceof RumorTold told) {
                tellings.add(new Telling(told.tellerId(), told.listenerId(),
                        told.newConfidence(), world.rumor(told.keptRumorId()).claim()));
            }
        }
        return tellings;
    }
