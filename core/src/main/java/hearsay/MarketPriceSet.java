 ...
 *
 * <p>Each good has its own market, so the event names the good it settled. Appended last so
 * the numbers that were already here stay where they were.
 *
 ...
 */
public record MarketPriceSet(long tick, int price, int askingVillagers, String item)
        implements Event {}
